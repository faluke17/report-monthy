...
function barFill(s: MnfAlertStatus) {
  if (s === 'red_spike' || s === 'red_accumulated') return '#B3392C'
  if (s === 'yellow') return '#A8721A'
  return '#0B6E76'
}
...
function NodeChart({ data, isLoading }: { data: ChartPt[]; isLoading: boolean }) {
  ...
  if (!data.length) return (
    <div className="h-52 flex items-center justify-center text-xs text-black/25">ยังไม่มีข้อมูลกราฟ (รัน mnf-sync ก่อน)</div>
  )
  return (
    <ResponsiveContainer width="100%" height={200}>
      <ComposedChart data={data} margin={{ top: 8, right: 8, left: -16, bottom: 0 }}>
        <CartesianGrid strokeDasharray="3 3" stroke="rgba(0,0,0,0.05)" vertical={false} />
        <XAxis dataKey="date" tick={{ fill: 'rgba(0,0,0,0.25)', fontSize: 9 }} tickLine={false} axisLine={false} interval={4} />
        <YAxis tick={{ fill: 'rgba(0,0,0,0.25)', fontSize: 9 }} tickLine={false} axisLine={false} width={36} />
        ...
        <Line type="monotone" dataKey="ema" name="ema" stroke="#1E7A5A" strokeWidth={1.5} dot={false} strokeDasharray="5 3" />
      </ComposedChart>
    </ResponsiveContainer>
  )
}
      ...
      <table className="w-full text-xs">
        ...
        <tbody>
          {sorted.map(n => {
            ...
            return (
              <tr key={n.node_label} className={`border-b border-black/5 transition-colors ${isHL ? 'bg-black/6' : isRed ? 'bg-red-500/5 hover:bg-red-500/8' : 'hover:bg-black/4'}`}>
                ...
                <td className="px-4 py-2.5 text-right text-black/70 num">{n.mnf_flow != null ? n.mnf_flow.toFixed(2) : '—'}</td>
  ...
  return (
    <div className="p-4 border-b border-black/8">
      ...
      <div className="flex items-center gap-4 mt-3 pt-3 border-t border-black/6 text-xs">
        <div><p className="text-black/30">MNF วันล่าสุด</p><p className="font-bold text-[#12181F] num">{node.mnf_flow != null ? node.mnf_flow.toFixed(2) : '—'} m³/hr</p></div>
        <div><p className="text-black/30">EMA-14</p><p className="font-bold text-black/60 num">{node.ema_value.toFixed(2)} m³/hr</p></div>
        <div>
        ...
        <div className="ml-auto"><p className="text-black/30">เกณฑ์</p><p className="text-black/35 text-[11px]">≥50% เฝ้าระวัง / ≥200% แดง</p></div>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="grid grid-cols-3 gap-3">
      <div className="glass-card p-4 border-l-4 border-red-500">
        ...
        <p className="text-xs text-black/35 mt-1">{unit} มี Alert</p>
      </div>
      <div className="glass-card p-4 border-l-4 border-amber-500">
        ...
        <p className="text-xs text-black/35 mt-1">{unit} MNF สูงกว่า EMA</p>
      </div>
      <div className="glass-card p-4 border-l-4 border-emerald-500">
        ...
        <p className="text-xs text-black/35 mt-1">{unit} ทุก node ปกติ</p>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="glass-card p-4">
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs font-bold text-black/40 uppercase tracking-widest">{groups.length} สาขา · {total} nodes</p>
      ...
      {selGroup && (
        <div key={selGroup.dmama_branch_id} className="glass-card overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-black/8">
            <div>
              <p className="text-sm font-bold text-[#12181F]">สาขา{selGroup.branch_name_th}</p>
              <p className="text-[11px] text-black/35 mt-0.5">{selGroup.nodes.length} nodes</p>
            </div>
            <button onClick={() => setSelectedId(null)} className="p-1.5 rounded-lg text-black/30 hover:text-black/60 hover:bg-black/6 transition-colors">
              <X size={14} />
            </button>
          </div>
          <ChartSection
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="glass-card overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-black/8">
          <div>
            <p className="text-sm font-bold text-[#12181F]">{rows[0]?.branch_name_th ?? ''} · {rows.length} nodes</p>
            <p className="text-[11px] text-black/35 mt-0.5">เลือก node เพื่อดูกราฟ 30 วัน</p>
          </div>
        </div>
        <ChartSection
          nodes={rows}
        ...
        />
        <div>
          <p className="px-4 pt-3 pb-1 text-[10px] text-black/25 font-bold uppercase tracking-widest">ทุก node</p>
