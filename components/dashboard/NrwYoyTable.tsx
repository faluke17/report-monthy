...
function RankBadge({ rank, delta, total }: { rank: number; delta: number | null; total: number }) {
  ...
  if (delta !== null && delta < -0.001 && rank <= 3) {
    const color = rank === 1
      ? 'text-emerald-200 bg-emerald-500/25 border-emerald-500/40 shadow-[0_0_6px_rgba(30,122,90,0.2)]'
      : rank === 2
  ...
  return <span className="text-black/25 num text-[11px] w-6 inline-block text-center">{rank}</span>
}
...
function DeltaCell({ delta, maxAbs }: { delta: number | null; maxAbs: number }) {
  ...
  return (
    <div className="flex items-center justify-end gap-2">
      <div className="w-10 h-0.5 rounded-full bg-black/8 relative overflow-hidden shrink-0">
        <div
      ...
      <span className={`num text-xs font-bold tracking-tight whitespace-nowrap ${
        isImproved ? 'text-emerald-400' : isWorsened ? 'text-red-400' : 'text-black/40'
      }`}>
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-black/8">
      <table className="w-full text-xs border-collapse min-w-[580px]">
        <thead>
          {/* Group headers */}
          <tr className="bg-black/3 border-b border-black/6">
            <th colSpan={2} className="px-2 py-1" />
            {/* น้ำสูญเสีย group */}
            <th colSpan={2} className="px-2 py-1 text-center border-l border-black/8">
              <span className="text-[9px] font-semibold text-black/40 tracking-wide">
            ...
            <th className="px-2 py-1 border-l border-black/8" />
          </tr>
          {/* Column headers */}
          <tr className="bg-black/5 border-b border-black/10">
            <th className="px-2 py-1.5 text-black/30 font-medium text-center w-7">#</th>
            <th className="px-2 py-1.5 text-left text-black/50 font-medium">สาขา</th>
            {/* น้ำสูญเสีย pair */}
            <th className="px-2 py-1.5 text-right text-blue-300/60 font-medium border-l border-black/8">
            ...
            {/* Delta */}
            <th className="px-2 py-1.5 text-right text-black/40 font-medium border-l border-black/8">
              เปลี่ยนแปลง
            </th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row, i) => (
            <tr
              key={row.branch_name}
              className={`border-b border-black/5 hover:bg-black/3 transition-colors ${rowTint(row.rate_delta)}`}
            >
              <td className="px-2 py-1 text-center">
                <RankBadge rank={i + 1} delta={row.rate_delta} total={rows.length} />
              </td>
              <td className="px-2 py-1 text-black/80 font-medium whitespace-nowrap">{row.branch_name}</td>
              {/* น้ำสูญเสีย pair */}
              <td className="px-2 py-1 text-right num text-blue-200/55 border-l border-black/5">{fmt(avgLoss(row.prev_loss), 0)}</td>
              <td className="px-2 py-1 text-right num text-orange-200/65">{fmt(avgLoss(row.curr_loss), 0)}</td>
              {/* NRW% pair */}
              <td className="px-2 py-1 text-right num text-blue-200/55 border-l border-black/5">{fmt(row.prev_rate)}</td>
              <td className="px-2 py-1 text-right num text-orange-200/65">{fmt(row.curr_rate)}</td>
              <td className="px-2 py-1 text-right border-l border-black/5">
        ...
        <tfoot>
          <tr className="border-t-2 border-black/12 bg-gradient-to-r from-black/5 to-transparent">
            <td className="px-2 py-2 text-black/20 text-center text-[9px]">x̄</td>
            <td className="px-2 py-2">
              <div className="text-black/50 font-semibold text-xs">ภาพรวมเขต</div>
              <div className="text-black/25 text-[9px] mt-0.5">
                {withCurr.length}/{rows.length} สาขา · {mc} เดือน
              </div>
            </td>
            {/* น้ำสูญเสีย pair */}
            <td className="px-2 py-2 text-right num text-blue-200/60 font-semibold border-l border-black/8">{fmt(distAvgPrevLoss, 0)}</td>
            <td className="px-2 py-2 text-right num text-orange-200/70 font-semibold">{fmt(distAvgCurrLoss, 0)}</td>
            {/* NRW% pair */}
            <td className="px-2 py-2 text-right num text-blue-200/60 font-semibold border-l border-black/8">{fmt(distPrevRate)}</td>
            <td className="px-2 py-2 text-right num text-orange-200/70 font-semibold">{fmt(distCurrRate)}</td>
            <td className="px-2 py-2 text-right border-l border-black/8">
