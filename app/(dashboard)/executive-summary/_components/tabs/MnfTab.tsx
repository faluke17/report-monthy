  ...
  return (
    <Card>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 3, minWidth: isMobile ? 480 : undefined }}>
        <div style={{ display: 'grid', gridTemplateColumns: gridCols, gap: 10, padding: '5px 10px', background: 'rgba(11,110,118,0.08)', borderBottom: `1px solid ${C.border}` }}>
        ...
        {mnfNodes.map((n, i) => {
          ...
          return (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: gridCols, gap: 10, padding: '9px 10px', background: red ? 'rgba(179,57,44,0.07)' : C.row, border: `1px solid ${red ? 'rgba(179,57,44,0.25)' : C.border}` }}>
              <div>
