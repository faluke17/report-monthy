...
export function BudgetTab({ budget }: { budget: BranchExecutiveSummary['budget_2569'] }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${isMobile ? 2 : 4},minmax(0,1fr))`, gap: 10 }}>
        {[
        ...
        <Card>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            {budget.projects.map((p, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', background: p.overdue ? 'rgba(179,57,44,0.07)' : C.row, border: `1px solid ${p.overdue ? 'rgba(179,57,44,0.25)' : C.border}` }}>
